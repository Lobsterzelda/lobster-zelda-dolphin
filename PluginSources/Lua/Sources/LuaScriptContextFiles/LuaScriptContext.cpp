...
#include "../CopiedScriptContextFiles/Enums/ScriptCallLocationsEnum.h"
...
namespace Scripting
{
  ...
  void* Init_LuaScriptContext_impl(void* new_base_script_context_ptr)
  {
    ...
    dolphinDefinedScriptContext_APIs.SetDerivedScriptContextPtr(new_lua_script_context_ptr->base_script_context_ptr, new_lua_script_context_ptr);
    ...
    std::string executionString =
      (std::string("package.path = package.path .. ';") + fileUtility_APIs.GetUserDirectoryPath() +
        "LuaLibs/?.lua;" + fileUtility_APIs.GetSystemDirectoryPath() + "LuaLibs/?.lua;'");
    ...
    void (*ImportModuleFuncPtr)(void*, const char*, const char*) = ((DLL_Defined_ScriptContext_APIs*)dolphinDefinedScriptContext_APIs.GetDLLDefinedScriptContextAPIs(new_lua_script_context_ptr->base_script_context_ptr))->ImportModule;
    const char* most_recent_script_version = dolphinDefinedScriptContext_APIs.GetScriptVersion();
    ...
    if (luaL_loadfile(new_lua_script_context_ptr->main_lua_thread, dolphinDefinedScriptContext_APIs.GetScriptFilename(new_lua_script_context_ptr->base_script_context_ptr)) != LUA_OK)
    {
      const char* temp_string = lua_tostring(new_lua_script_context_ptr->main_lua_thread, -1);
      dolphinDefinedScriptContext_APIs.GetPrintCallbackFunction(new_lua_script_context_ptr->base_script_context_ptr)(new_lua_script_context_ptr->base_script_context_ptr, temp_string);
      dolphinDefinedScriptContext_APIs.GetScriptEndCallbackFunction(new_lua_script_context_ptr->base_script_context_ptr)(new_lua_script_context_ptr->base_script_context_ptr, dolphinDefinedScriptContext_APIs.GetUniqueScriptIdentifier(new_lua_script_context_ptr->base_script_context_ptr));
    }
    else
    {
      dolphinDefinedScriptContext_APIs.AddScriptToQueueOfScriptsWaitingToStart(new_lua_script_context_ptr->base_script_context_ptr);
    }
  ...
  {
    LuaScriptContext* lua_script_ptr = (LuaScriptContext*)dolphinDefinedScriptContext_APIs.GetDerivedScriptContextPtr(base_script_context_ptr);
  ...
  void SetErrorCode(void* base_script_context_ptr, lua_State* lua_state, const char* error_str)
  {
    ...
    dolphinDefinedScriptContext_APIs.SetErrorMessage(base_script_context_ptr, error_str);
  ...
  int CustomPrintFunction(lua_State* lua_state)
  {
    ...
    dolphinDefinedScriptContext_APIs.GetPrintCallbackFunction(corresponding_script_context)(corresponding_script_context, output_string.c_str());
  ...
  {
    if (dolphinDefinedScriptContext_APIs.GetIsFinishedWithGlobalCode(lua_script_ptr->base_script_context_ptr) &&
  ...
  {
    LuaScriptContext* lua_script_ptr = reinterpret_cast<LuaScriptContext*>(dolphinDefinedScriptContext_APIs.GetDerivedScriptContextPtr(base_script_ptr));
    Scripting::ScriptCallLocationsEnum call_location = (Scripting::ScriptCallLocationsEnum)(dolphinDefinedScriptContext_APIs.GetScriptCallLocation(base_script_ptr));
    lua_State* current_lua_thread = lua_script_ptr->main_lua_thread;

    if (call_location == Scripting::ScriptCallLocationsEnum::FromFrameStartCallback)
      current_lua_thread = lua_script_ptr->frame_callback_lua_thread;

    else if (call_location == Scripting::ScriptCallLocationsEnum::FromInstructionHitCallback)
      current_lua_thread = lua_script_ptr->instruction_address_hit_callback_lua_thread;

    else if (call_location == Scripting::ScriptCallLocationsEnum::FromMemoryAddressReadFromCallback)
      current_lua_thread = lua_script_ptr->memory_address_read_from_callback_lua_thread;

    else if (call_location == Scripting::ScriptCallLocationsEnum::FromMemoryAddressWrittenToCallback)
      current_lua_thread = lua_script_ptr->memory_address_written_to_callback_lua_thread;

    else if (call_location == Scripting::ScriptCallLocationsEnum::FromGCControllerInputPolledCallback)
      current_lua_thread = lua_script_ptr->gc_controller_input_polled_callback_lua_thread;

    else if (call_location == Scripting::ScriptCallLocationsEnum::FromWiiInputPolledCallback)
    ...
    *userdata_ptr_ptr = GetNewUserdataInstance();
    ...
    for (auto& current_function_metadata : lua_script_ptr->class_metadata_buffer.functions_list)
    {
      lua_CFunction lambdaFunction = [](lua_State* lua_state) mutable {
        ...
        lua_getglobal(lua_state, THIS_SCRIPT_CONTEXT_VARIABLE_NAME);
        void* base_script_context_ptr = lua_touserdata(lua_state, -1);
        LuaScriptContext* lua_script_context_ptr = (LuaScriptContext*)dolphinDefinedScriptContext_APIs.GetDerivedScriptContextPtr(base_script_context_ptr);
        lua_pop(lua_state, 1);
        ...
        if (localFunctionMetadata->return_type == Scripting::ArgTypeEnum::U64)
        {
        ...
        for (Scripting::ArgTypeEnum arg_type : localFunctionMetadata->arguments_list)
        {
          switch (arg_type)
          {
          case Scripting::ArgTypeEnum::Boolean:
            ...
            vectorOfArgHolder_APIs.PushBack_VectorOfArgHolders(arguments_list_ptr, temp_arg_holder_ptr);
            break;
          ...
          case Scripting::ArgTypeEnum::RegistrationForButtonCallbackInputType:
            if (!((DLL_Defined_ScriptContext_APIs*)dolphinDefinedScriptContext_APIs.GetDLLDefinedScriptContextAPIs(base_script_context_ptr))->IsButtonRegistered(base_script_context_ptr,
          ...
          case Scripting::ArgTypeEnum::ListOfBytes:
            temp_arg_holder_ptr = argHolder_APIs.CreateBytesArgHolder();
            ...
            while (lua_next(lua_state, next_index_in_args))
            {
              ...
              argHolder_APIs.AddByteToBytesArgHolder(temp_arg_holder_ptr, value);
              lua_pop(lua_state, 1);
            }
            vectorOfArgHolder_APIs.PushBack_VectorOfArgHolders(arguments_list_ptr, temp_arg_holder_ptr);
            break;
          ...
          case Scripting::ArgTypeEnum::GameCubeControllerStateObject:
            lua_pushnil(lua_state);
            temp_arg_holder_ptr = argHolder_APIs.CreateGameCubeControllerStateArgHolder();
            while (lua_next(lua_state, next_index_in_args) != 0)
            {
              ...
              argHolder_APIs.SetGameCubeControllerStateArgHolderValue(temp_arg_holder_ptr, button_name_as_enum, button_value);
              lua_pop(lua_state, 1);
            }
            vectorOfArgHolder_APIs.PushBack_VectorOfArgHolders(arguments_list_ptr, temp_arg_holder_ptr);
            break;
        ...
        case Scripting::ArgTypeEnum::ShutdownType:
          ...
          return 0;
        case Scripting::ArgTypeEnum::ListOfBytes:
          size_of_bytes_list = argHolder_APIs.GetSizeOfBytesArgHolder(return_value);
          lua_createtable(lua_state, static_cast<int>(size_of_bytes_list), 0);

          key = 1;

          for (size_t i = 0; i < size_of_bytes_list; ++i)
          {
            value = argHolder_APIs.GetByteFromBytesArgHolderAtIndex(return_value, i);
        ...
        case Scripting::ArgTypeEnum::GameCubeControllerStateObject:
          ...
          for (int next_digital_button : digital_buttons_list)
          {
            ...
            lua_pushboolean(lua_state, argHolder_APIs.GetGameCubeControllerStateArgHolderValue(return_value, next_digital_button));
          ...
          for (int next_analog_button : analog_buttons_list)
          {
            ...
            lua_pushstring(lua_state, button_name);
            lua_pushinteger(lua_state, argHolder_APIs.GetGameCubeControllerStateArgHolderValue(return_value, next_analog_button));
            lua_settable(lua_state, -3);
          }
          return freeAndReturn(return_value, 1);
  ...
  {
    dolphinDefinedScriptContext_APIs.GetPrintCallbackFunction(base_script_context_ptr)(base_script_context_ptr, string_to_print);
  }

  LuaScriptContext* getLuaScriptContext(void* base_script_context_ptr)
  {
    return reinterpret_cast<LuaScriptContext*>(dolphinDefinedScriptContext_APIs.GetDerivedScriptContextPtr(base_script_context_ptr));
  }
  void StartScript_impl(void* base_script_context_ptr)
  {
    if (!dolphinDefinedScriptContext_APIs.GetIsScriptActive(base_script_context_ptr)) // this variable is set to true when a Script is first created (in its constructor)
    {
    ...
    if (retVal == LUA_YIELD)
      dolphinDefinedScriptContext_APIs.SetCalledYieldingFunctionInLastGlobalScriptResume(base_script_context_ptr, 1);
    else
    {
      dolphinDefinedScriptContext_APIs.SetCalledYieldingFunctionInLastGlobalScriptResume(base_script_context_ptr, 0);
      if (retVal == LUA_OK)
      {
        dolphinDefinedScriptContext_APIs.SetIsFinishedWithGlobalCode(base_script_context_ptr, 1);
  ...
  {
    if (!dolphinDefinedScriptContext_APIs.GetIsScriptActive)
      return true;
    ...
    int& index_of_next_callback_to_run, bool is_frame_start_callback)
  {
    ...
    {
      if (is_frame_start_callback && dolphinDefinedScriptContext_APIs.GetCalledYieldingFunctionInLastFrameCallbackScriptResume(base_script_context_ptr))
      {
      ...
      {
        dolphinDefinedScriptContext_APIs.SetCalledYieldingFunctionInLastFrameCallbackScriptResume(base_script_context_ptr, 1);
        break;
      }
      else
      {
        if (is_frame_start_callback)
          dolphinDefinedScriptContext_APIs.SetCalledYieldingFunctionInLastFrameCallbackScriptResume(base_script_context_ptr, 0);
        if (ret_val != LUA_OK)
        {
  ...
  void DLLClassMetadataCopyHook_impl(void* base_script_context_ptr, void* class_metadata_ptr)
  {
    ...
    ((LuaScriptContext*)dolphinDefinedScriptContext_APIs.GetDerivedScriptContextPtr(base_script_context_ptr))->class_metadata_buffer = ClassMetadata(class_name, functions_list);
  }
  ...
  void DLLFunctionMetadataCopyHook_impl(void* base_script_context_ptr, void* function_metadata_ptr)
  {
    ...
    for (unsigned long long arg_index = 0; arg_index < num_args; ++arg_index)
      argument_type_list.push_back((Scripting::ArgTypeEnum)functionMetadata_APIs.GetTypeOfArgumentAtIndex(function_metadata_ptr, arg_index));
    ((LuaScriptContext*)dolphinDefinedScriptContext_APIs.GetDerivedScriptContextPtr(base_script_context_ptr))->single_function_metadata_buffer =
